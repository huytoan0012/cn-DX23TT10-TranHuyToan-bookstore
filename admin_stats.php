...
while($row = $chartResult->fetch_assoc()) {
    if ($filter == 'month') {
        ...
        $orderCounts[] = $row['daily_orders'];
    } elseif ($filter == 'year') {
        $labels[] = "Tháng " . $row['month'];
        $values[] = $row['monthly_total'];
        $orderCounts[] = $row['monthly_orders'];
    } else {
        $labels[] = $row['date'];
        $values[] = $row['daily_total'];
        $orderCounts[] = $row['daily_orders'];
...
console.log('Order counts:', <?= json_encode($orderCounts) ?>);
</script>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
// Chuyển đổi dữ liệu sang số
var rawLabels = <?= json_encode($labels) ?>;
var rawValues = <?= json_encode($values) ?>;
var rawOrders = <?= json_encode($orderCounts) ?>;

// Đảm bảo dữ liệu là số
var chartValues = rawValues.map(v => parseFloat(v) || 0);
var chartOrders = rawOrders.map(v => parseInt(v) || 0);

console.log('Labels:', rawLabels);
console.log('Doanh thu:', chartValues);
console.log('Đơn hàng:', chartOrders);

const ctx = document.getElementById('revenueChart').getContext('2d');
const revenueChart = new Chart(ctx, {
    type: 'bar',  // Đổi sang bar cho dễ nhìn hơn
    data: {
        labels: rawLabels,
        datasets: [
            {
